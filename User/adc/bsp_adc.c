...
static void ADCx_GPIO_Config(void)
{
	...
	macADC_GPIO_APBxClock_FUN ( macADC_GPIO_CLK_B, ENABLE );
	// 配置 ADC IO 引脚模式
	// ADC 3个通道
	GPIO_InitStructure.GPIO_Pin = macADC_PIN1|macADC_PIN2|macADC_PIN3|macADC_PIN4|macADC_PIN5|macADC_PIN6;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;
	// 初始化 ADC IO
	GPIO_Init(GPIOC, &GPIO_InitStructure);			
	
	GPIO_InitStructure.GPIO_Pin = macADC_PINB_1|macADC_PINB_2;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;
	// 初始化 ADC IO
...
static void ADCx_Mode_Config(void)
{
	...
	ADC_RegularChannelConfig(macADCx, macADC_CHANNEL7, 7, ADC_SampleTime_55Cycles5);
	ADC_RegularChannelConfig(macADCx, macADC_CHANNEL8, 8, ADC_SampleTime_55Cycles5);
